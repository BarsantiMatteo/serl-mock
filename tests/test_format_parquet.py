...
def test_edition09_output_has_same_structure_as_edition08(
    ...
    #
    # Daily and HH smart-meter files are excluded: edition09 deliberately
    # uses a real, different column schema for both ("harmonised" — see
    # src/serl_mock/edition.py's smart_meter_schema and
    # tests/test_edition09_smart_meter_schema.py for that comparison
    # instead), and a different daily-smart-meter layout ("single_file", no
    # subfolder) than edition08 ("yearly", in its own subfolder) — see
    # tests/test_daily_layout_edition09.py for that comparison.
    ...
    def normalize(files):
        ...
        for rel, shape in files.items():
            if "smart_meter_daily" in rel or "smart_meter_hh" in rel:
                continue
